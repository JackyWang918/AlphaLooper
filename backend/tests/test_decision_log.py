...
def test_paused_order_watch_never_writes_strategy_logs(rig):
    r = rig
    tick(r)
    ...
    tick(r, 60)
    assert decision_log.read(r.auto.engine, task_id, kind="order_check")["items"] == []

    # Read-only reconciliation can update state, but remains outside the
    # strategy decision log until the user explicitly resumes.
    r.browser.partial("1")
    tick(r, 60)
    assert decision_log.read(r.auto.engine, task_id, kind="order_check")["items"] == []
    assert decision_log.read(r.auto.engine, task_id, kind="error")["items"] == []

    r.browser.finish()
    tick(r, 60)
    tick(r, 5)
    assert decision_log.read(r.auto.engine, task_id, kind="fill")["items"] == []
