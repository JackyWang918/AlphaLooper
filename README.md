...
基础工程已初始化，控制台支持启动独立持久化 Chrome、打开指定 Alpha 链接和刷新浏览器状态。浏览器由独立子进程管理，页面操作串行执行。SQLite 与 Alembic 迁移入口可运行。

已按中文 Alpha 页面的实际控件实现“仅填表并核对”，支持识别 USDT/USDC 计价币、切换买卖方向、填写价格和数量，再回读核对。未包含订单提交或策略功能。控制台中的操作反馈仅保留在当前页面内存中；刷新后重新查询浏览器状态，不重放操作。
