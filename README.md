...
执行 `uv run alembic upgrade head` 更新至迁移 0004，再启动后端。在独立 Chrome 登录并打开历史委托，展开需要读取的订单以显示订单 ID，点击控制台“读取订单汇总并更新账本”。展开区域仅取 ID，不解析逐笔成交。当前不会自动展开、翻页或下单；缺少 ID 的行会提示跳过。
...
切换平台账户时使用不同的账户账本名称，程序不自动识别账户身份。统计只代表已读取范围，按链/页面合约/币种/计价币分组累计，不自动按日重置。预计已实现盈亏按订单创建顺序及平均买入成本估算，缺失买入成本、同秒订单顺序不明或存在未结束订单时显示未知；不含持仓浮盈亏，也不是平台核定净盈亏。所有记录保存在被 Git 忽略的本机 `data/` 数据库，原始快照仅供排查。
