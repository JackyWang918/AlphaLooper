...
type Intent={id:string;created_at:number;state:string;message:string;submission_error?:string;last_check_error?:string;request:{side:string;price:string;quantity:string;quote_amount?:string;sell_all?:boolean;expected_symbol:string};result?:{status:string;gross:string;quantity:string};active:boolean}
...
const states:Record<string,string>={preparing:'提交前核对',not_submitted:'未提交',submission_unknown:'提交结果待核实',waiting:'等待平台结果',settling:'核对余额',completed:'已结束并记账'}
...
async function submit() {
  ...
  try {
    await request('orders',{request_id:requestId.value,book:book.value,url:props.url,side:side.value,price:price.value,quantity:'1',quote_amount:side.value==='buy'?quantity.value:undefined,sell_all:side.value==='sell',expected_symbol:props.symbol,expected_quote:props.quote})
    await refresh()
...
async function checkReadiness() {
  busy.value=true;error.value='';readiness.value='正在检查当前委托和可用余额，不填写或提交…'
  try {
    const response=await fetch('/api/browser/order-readiness',{method:'POST',headers:{'Content-Type':'application/json','X-AlphaLooper-Client':'local-ui'},body:JSON.stringify({url:props.url,side:side.value,price:price.value,quantity:'1',quote_amount:side.value==='buy'?quantity.value:undefined,sell_all:side.value==='sell',expected_symbol:props.symbol,expected_quote:props.quote}),signal:AbortSignal.timeout(55000)})
    const data=await response.json()
    if(!response.ok) throw new Error(typeof data.detail==='string'?data.detail:'请填写有效价格和金额，并确认表单已识别。')
    readiness.value='订单读取检查通过：当前无挂单，两侧余额已读取。未填写、未提交订单。'
...
<section>
  ...
  <p class="muted">提交前会先检查当前无挂单，再保存两侧可用余额作核对依据。可先使用下方只读检查定位页面问题。</p>
  <div class="fields">
  ...
  <button class="secondary" :disabled="busy||browserBusy||!!current||!fillSupported||!symbol||!quote||!!inputProblem" @click="checkReadiness">仅检查订单读取（不下单）</button>
  ...
  <div v-if="current" class="notice">
    <p>如果没有完成平台二次确认：先手动关闭确认弹窗，核对当前无挂单、余额未变化，再解除本地等待。</p>
